...
int main(int argc, char *argv[]) 
{
    ...
    if (  ! createDirWithPermissionAndOwnership(@"/Library/Application Support/SurfSafeVPN",
                                                0755, 0, 0)  ) {
                errorExit();
            }
...
//**************************************************************************************************************************
    ...
    */
    ...
    if ( secureApp ) {
        ...
        if (  ! okSoFar  ) {
            appendLog(@"Unable to secure SurfSafeVPN.app");
            errorExit();
        }
    }
        ...
        if (  [gFileMgr fileExistsAtPath: CONFIGURATION_UPDATES_BUNDLE_PATH]  ) {
            ...
            if (  libVersion  ) {
                if (  ! [libVersion isEqualToString: bundleVersion]  ) {
                    ...
                    changed = TRUE;
                    appendLog([NSString stringWithFormat: @"SurfsafeVPN Configurations.bundle CFBundleVersion has been set to %@", bundleVersion]);
                } else {
                    appendLog([NSString stringWithFormat: @"SurfsafeVPN Configurations.bundle CFBundleVersion is %@", bundleVersion]);
                }
            } else {
            ...
            if (  libVersion  ) {
                if (  ! [libVersion isEqualToString: bundleShortVersionString]  ) {
                    ...
                    changed = TRUE;
                    appendLog([NSString stringWithFormat: @"SurfsafeVPN Configurations.bundle CFBundleShortVersionString has been set to %@", bundleShortVersionString]);
                } else {
                    appendLog([NSString stringWithFormat: @"SurfsafeVPN Configurations.bundle CFBundleShortVersionString is %@", bundleShortVersionString]);
                }
            } else {
